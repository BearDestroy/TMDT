...
import { layDSPhuongThucThanhToan, xoaPhuongThucThanhToan } from '@/apis/phuongthucthanhtoan'
import { useMutation, useQuery } from '@tanstack/react-query'
import { useQueryString } from '@/hooks/use-query-string'
import { useLocation, useNavigate } from 'react-router-dom'
import PhuongThucThanhToanForm from '@/components/Form/PhuongThucThanhToanForm'
...
export function QuanLyPhuongThucThanhToan() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey: ['dsPhuongThucThanhToan', soTrang, soLuong, trangThai, tuKhoa],
    queryFn: () => layDSPhuongThucThanhToan(soTrang, soLuong, trangThai, tuKhoa)
  })
  ...
  const [editingPhuongThucThanhToan, setEditingPhuongThucThanhToan] = useState<phuongThucThanhToanFormData | undefined>(
    undefined
  )
  const [selectedRowId, setSelectedRowId] = useState<number | null>(null)
  const handleSetEditingPhuongThucThanhToan = (data: PhuongThucThanhToan) => {
    setEditingPhuongThucThanhToan({
      tenPhuongThucThanhToan: data.tenPhuongThucThanhToan,
      maPhuongThucThanhToan: data.maPhuongThucThanhToan,
  ...
  const handleFormSuccess = () => {
    ...
    setEditingPhuongThucThanhToan(undefined)
    refetch()
  }
  ...
  const { mutate: deleteMutate, isPending: isDeleting } = useMutation({
    mutationFn: ({ id }: { id: number }) => xoaPhuongThucThanhToan(id),
    onSuccess: (data) => {
      ...
      <div className='flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 w-full'>
        <div className='lg:col-span-2 bg-white rounded-lg border border-gray-200 p-6 w-full shadow-sm flex flex-col'>
          <div className='space-y-3'>
            <div className='flex gap-2'>
              <Input
                placeholder='Tìm kiếm theo mã hoặc tên phương thức thanh toán...'
                value={searchQuery}
                ...
                {ketQua.length === 0 ? (
                  ...
                  ketQua.map((s, i) => (
                    <TableRow
                      ...
                      onClick={() => {
                        handleSetEditingPhuongThucThanhToan(s)
                    ...
                    >
                      ...
                      <TableCell className='text-center border text-base py-0.5 border-gray-200 border-b '>
                        {s.maPhuongThucThanhToan}
                      </TableCell>
                      <TooltipTableCell text={s.tenPhuongThucThanhToan} index={i} className='text-base py-3' />
                      <TableCell className='text-center border text-base py-0.5 border-gray-200 border-b '>
        ...
        <div className='bg-white rounded-lg border p-6 w-full shadow-sm'>
          <PhuongThucThanhToanForm
            defaultValues={editingPhuongThucThanhToan}
