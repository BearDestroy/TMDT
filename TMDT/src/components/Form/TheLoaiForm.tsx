...
import { suaTheLoai, themTheLoai } from '@/apis/theloai'
...
import { theLoaiSchema, type theLoaiFormData } from '@/validations/theloai.schema'
import { Command, CommandEmpty, CommandGroup, CommandInput, CommandItem, CommandList } from '@/components/ui/command'
import { Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover'

interface Props {
  defaultValues?: theLoaiFormData
  editingId?: number | null
  setSelectRowId: (data: number | null) => void
  onSuccess?: () => void
}

export default function TheLoaiForm({ defaultValues, editingId, setSelectRowId, onSuccess }: Props) {
  ...
  const { mutate: addMutate, isPending: isAdding } = useMutation({
    mutationFn: (body: theLoaiFormData) => themTheLoai(body),
    onSuccess: (data) => {
  ...
  const { mutate: updateMutate, isPending: isUpdating } = useMutation({
    mutationFn: ({ id, body }: { id: number; body: theLoaiFormData }) => suaTheLoai(body, id),
    onSuccess: (data) => {
      if (data.statusCode === 200) {
        showSuccessToast({ message: data.message })
        onSuccess?.()
  ...
  const {
    ...
    reset,
    formState: { errors }
  } = useForm<theLoaiFormData>({
    resolver: zodResolver(theLoaiSchema),
    defaultValues: defaultValues || { maTheLoai: '', tenTheLoai: '', trangThai: true, idDanhMuc: 0 },
    mode: 'onBlur',
    reValidateMode: 'onSubmit'
  })

  const onSubmit = (data: theLoaiFormData) => {
    if (editingId) {
  ...
  const handleReset = () => {
    reset({ maTheLoai: '', tenTheLoai: '', trangThai: true, idDanhMuc: 0 })
    setSelectRowId(null)
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className='space-y-4'>
      <fieldset disabled={isSubmitting} className='space-y-4'>
        <div>
          ...
          />
          {errors.maTheLoai && <p className='text-red-600 text-base mt-1'>{errors.maTheLoai.message}</p>}
        </div>
        ...
        <div>
          ...
          />
          {errors.tenTheLoai && <p className='text-red-600 text-base mt-1'>{errors.tenTheLoai.message}</p>}
        </div>
          ...
          <Controller
            ...
            render={({ field }) => (
              <div>
                  ...
                  <PopoverTrigger asChild>
                    ...
                    >
                      {loadingDM
                        ? 'Đang tải...'
                        : field.value
                          ? danhMucs.find((dm) => dm.id === field.value)?.tenDanhMuc
                          : 'Chọn danh mục'}
                ...
                {errors.idDanhMuc && <p className='text-red-600 text-base mt-1'>{errors.idDanhMuc.message}</p>}
              </div>
            )}
          />
        </div>
