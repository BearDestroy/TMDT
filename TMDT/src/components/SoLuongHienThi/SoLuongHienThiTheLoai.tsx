...
import { createURLTheLoai } from '@/utils/function'
...
export default function FooterTableTheLoai({
  soTrangHienTai,
  soLuongMoiTrang,
  trangThai,
  tongSoLuong,
  ...
  return (
    <div className='flex items-center justify-between'>
      <Select
        ...
        onValueChange={(v) => {
          const num = Number(v)
          navigate(createURLTheLoai(location.pathname, 1, num, trangThai, tuKhoa, idDanhMuc))
        }}
      >
      ...
      <div className='flex items-center gap-1'>
        <Button
          ...
          onClick={() => {
            if (soTrangHienTai > 1) {
              navigate(createURLTheLoai(pathname, soTrangHienTai - 1, soLuongMoiTrang, trangThai, tuKhoa, idDanhMuc))
            }
          }}
        >
          <ChevronLeft />
        ...
        {getPageNumbers(soTrangHienTai, tongSoTrang).map((p, idx) =>
          typeof p === 'number' ? (
            <Button
              ...
              size='icon'
              onClick={() => navigate(createURLTheLoai(pathname, p, soLuongMoiTrang, trangThai, tuKhoa, idDanhMuc))}
