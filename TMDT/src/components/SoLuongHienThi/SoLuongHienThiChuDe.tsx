  ...
  return (
    <div className='flex items-center justify-between'>
      <Select
        ...
        onValueChange={(v) => {
          const num = Number(v)
          navigate(createURLChuDe(location.pathname, 1, num, trangThai, tuKhoa, idTheLoai))
        }}
      >
      ...
      <div className='flex items-center gap-1'>
        <Button
          ...
          onClick={() => {
            if (soTrangHienTai > 1) {
              navigate(createURLChuDe(pathname, soTrangHienTai - 1, soLuongMoiTrang, trangThai, tuKhoa, idTheLoai))
            }
          }}
        >
          <ChevronLeft />
        ...
        {getPageNumbers(soTrangHienTai, tongSoTrang).map((p, idx) =>
          typeof p === 'number' ? (
            <Button
              ...
              size='icon'
              onClick={() => navigate(createURLChuDe(pathname, p, soLuongMoiTrang, trangThai, tuKhoa, idTheLoai))}
